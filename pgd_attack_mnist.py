...
parser.add_argument('--model-path',
                    ...
                    #default='./model-mnist-smallCNN/model-nn-epoch-b610.pt',
...
# set up data loader
transform_test = transforms.Compose([transforms.ToTensor(),])#compose串联多个变换
...
def plot_dataset_digits(dataset, name, k):
    ...
    for i in range(columns * rows):
        ...
        ax[-1].set_title("MNIST")  # set title
        plt.imshow(img)


    # plt.show()  # finally, render the plot
    plt.savefig('./results/img'+name+'{}'.format(k)+'.png')
def _red_whitebox(model,
                  X,
                  y,
                  k,
    ...
    for _ in range(num_steps):
        ...
        X_pgd = Variable(X_pgd.data + eta, requires_grad=True)#前面有random的noise
    ...
    plot_dataset_digits(X.cpu().detach().numpy().squeeze(), 'natural',k)
    plot_dataset_digits(X_pgd.cpu().detach().numpy().squeeze(), 'adv',k)
    plot_dataset_digits(X_p.cpu().detach().numpy().squeeze(), 'red',k)
...
def eval_adv_test_whitebox(model, device, test_loader):
    ...
    natural_err_total = 0
    attack_err_total = 0
    recovered_err_total = 0
    k=0
    for data, target in test_loader:
        data, target = data.to(device), target.to(device)
        # pgd attack
        X, y = Variable(data, requires_grad=True), Variable(target)
        delta_per,err_natural, err_recovered, err_attack = _red_whitebox(model, X, y,k)
    ...
    print('attack_err_total: ', attack_err_total)
    print('recovered_err_total: ', recovered_err_total)
